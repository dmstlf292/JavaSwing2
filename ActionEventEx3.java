...
public class ActionEventEx3 extends MFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) { // 버튼4개는 cmd로 구분!
		...
		if(cmd.equals(lab[0]) || tf ==e.getSource()) { // 엑션이벤트 : 추가 또는 키이벤트 : 엔터치면 입력되는 이벤트 
			...
			if(item.trim().length()==0) {// 입력을 한 값이 공백이라면 , trim 빈값을 제거시키는 기능 
				MDialog md = new MDialog(this, "알림","문자열을 입력하세요");//조매난 알림창 뜨는기능!!!
				int width = 180;
				int height = 100;
				int px = getX();
				int py = getY();
				...
				md.setVisible(true);
				return; // 중요, 반복문 빠져나오는건 break + 메소드 빠져나오는건 return
			}
	...
	class MDialog extends Dialog implements ActionListener{ //-----------1먼저 선언
		
		Button btn;

		public MDialog(Frame f, String title, String msg) {
			super(f,title,true);
			setLayout(new FlowLayout()); // FlowLayout이 뭐였더라?
			add(new Label(msg, Label.CENTER)); //대체 Label은 어디에 선언되어있는가?
